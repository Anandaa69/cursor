...
def scan_current_node_absolute(gimbal, chassis, sensor, tof_handler, graph_mapper, marker_handler=None, ep_robot=None):
    ...
    time.sleep(0.2)
     ...
     # ตรวจสอบว่าทิศทางซ้ายอยู่นอกขอบเขตแมพหรือไม่
     left_direction_map = {
         'north': 'west',
         'south': 'east', 
         'east': 'north',
     ...
     left_absolute_direction = left_direction_map[graph_mapper.currentDirection]
     left_target_pos = graph_mapper.get_next_position(left_absolute_direction)
     
     if not graph_mapper.is_position_within_boundaries(left_target_pos):
         print(f"🚫 LEFT direction leads to {left_target_pos} which is OUTSIDE map boundaries!")
         print(f"🗺️ Map boundaries: x[{graph_mapper.min_x},{graph_mapper.max_x}], y[{graph_mapper.min_y},{graph_mapper.max_y}]")
         # จำลองการเจอกำแพงที่ขอบแมพ
         left_distance = 30.0   # ระยะปลอม
         left_wall = True       # บังคับให้เป็นกำแพง
         scan_results['left'] = left_distance
         print(f"📏 LEFT ToF result: {left_distance:.2f}cm - WALL (BOUNDARY)")
     else:
         # ToF scan ปกติ
    ...
    if graph_mapper.currentPosition == (0, 0) and current_node.initialScanDirection == graph_mapper.currentDirection:
        ...
        # ตรวจสอบว่าทิศทางหลังอยู่นอกขอบเขตแมพหรือไม่
        back_direction_map = {
            'north': 'south',
            'south': 'north',
            'east': 'west',
        ...
        back_absolute_direction = back_direction_map[graph_mapper.currentDirection]
        back_target_pos = graph_mapper.get_next_position(back_absolute_direction)

        if not graph_mapper.is_position_within_boundaries(back_target_pos):
            print(f"🚫 BACK direction leads to {back_target_pos} which is OUTSIDE map boundaries!")
            print(f"🗺️ Map boundaries: x[{graph_mapper.min_x},{graph_mapper.max_x}], y[{graph_mapper.min_y},{graph_mapper.max_y}]")
            # จำลองการเจอกำแพงที่ขอบแมพ
            back_distance = 30.0   # ระยะปลอม
            back_wall = True       # บังคับให้เป็นกำแพง
            scan_results['back'] = back_distance
            print(f"📏 BACK ToF result: {back_distance:.2f}cm - WALL (BOUNDARY)")
        else:
            # ToF scan for back ปกติ
        ...
        back_abs_dir = back_absolute_direction
